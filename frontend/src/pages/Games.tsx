    ...
    return (
        <div className="flex w-full justify-center flex-col items-center">
            <div className="w-1/2 items-center text-center my-10 border-2 rounded-md p-4 text-white bg-sky-500">
                ...
                <h2 className="text-2xl font-semibold">Here you can find all the games we have to offer!</h2>
            </div>
            <div className="flex w-2/3 justify-center">
                { games.map(( game, i ) => <GameCard { ...game } key={ i } onClick={ onClickGame }/>) }
            </div>
        </div>
    );
}
...
const GameCard:React.FC<GameCardProps> = ({ name, description, image, route, onClick }) => {
    return (
        <div
